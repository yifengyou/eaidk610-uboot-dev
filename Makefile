...
u-boot-rockchip.bin: idbloader.img u-boot.itb FORCE
	$(call if_changed,pad_cat)
ifneq ($(src), $(obj))
# YYF 如果src目录，与目标目录不同，则需要拷贝rk_tools过去，否则生成镜像会缺失依赖
	@echo "copy $(src)/rk_tools to $(obj)/tools" && cp -a ${src}/tools/rk_tools ./tools
endif
	@echo "generate rk3399_loader_v1.12.112.bin"
	chmod +x tools/rk_tools/tools/boot_merger
	tools/rk_tools/tools/boot_merger tools/rk_tools/RKBOOT/RK3399MINIALL.ini
	@echo ""

	@echo "generate trust.img"
	chmod +x tools/rk_tools/tools/trust_merger
	tools/rk_tools/tools/trust_merger tools/rk_tools/RKTRUST/RK3399TRUST.ini
	@echo ""

	@echo "generate uboot.img"
	chmod +x tools/rk_tools/tools/loaderimage
	tools/rk_tools/tools/loaderimage --pack --uboot u-boot.bin uboot.img
	@echo ""
...
CLEAN_FILES += include/bmp_logo.h include/bmp_logo_data.h \
	       ...
	       itb.fit.fit itb.fit.itb itb.map spl.map uboot.img trust.bin
